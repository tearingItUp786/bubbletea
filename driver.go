...
func (d *driver) ReadInput() ([]Msg, error) {
...
func (d *driver) PeekInput() ([]Msg, error) {
...
func (d *driver) peekInput() (int, []Msg, error) {
	ev := make([]Msg, 0)
	p, err := d.rd.Peek(1)
	if err != nil {
		return 0, nil, err
	...
	for i < len(p) {
		...
		if e != nil {
			...
			case pasteEndMsg:
				...
				ev = append(ev, KeyMsg{Runes: paste, Paste: true})
			}
			ev = append(ev, e)
		}
		i += nb
...
func (d *driver) peekOne(p []byte) (int, Msg) {
	if len(p) == 0 {
		return 0, nil
	}
	...
	var parser func(int, []byte, bool) (int, Msg)
...
begin:
	...
	return 1, UnknownSequenceMsg(string(p[0]))
}

func (d *driver) parseCsi(i int, p []byte, alt bool) (int, Msg) {
	var seq []byte
	...
	if i >= len(p) || p[i] < 0x40 || p[i] > 0x7E {
		...
		var e Msg = UnknownSequenceMsg(seq)
	...
	case initial == '?' && cmd == 'u' && len(params) > 0:
		...
		return len(seq), KittyKeyboardMsg(params[0][0])
	case initial == '?' && cmd == 'c':
	...
	case string(seq) == "\x1b[200~":
		...
		return len(seq), pasteStartMsg{}
	case string(seq) == "\x1b[201~":
		// bracketed-paste end
		return len(seq), pasteEndMsg{}
	}

	return len(seq), UnknownCsiMsg{csi}
}

// parseSs3 parses a SS3 sequence.
// See https://vt100.net/docs/vt220-rm/chapter4.html#S4.4.4.2
func (d *driver) parseSs3(i int, p []byte, alt bool) (int, Msg) {
	var seq []byte
	...
	return len(seq), UnknownSequenceMsg(seq)
}

func (d *driver) parseOsc(i int, p []byte, _ bool) (int, Msg) {
	var seq []byte
...
func (d *driver) parseCtrl(intro8, intro7 byte) func(int, []byte, bool) (int, Msg) {
	return func(i int, p []byte, _ bool) (int, Msg) {
		var seq []byte
...
func (d *driver) parseDcs(i int, p []byte, alt bool) (int, Msg) {
	// DCS sequences are introduced by DCS (0x90) or ESC P (0x1b 0x50)
	return d.parseCtrl(ansi.DCS, 'P')(i, p, alt)
}

func (d *driver) parseApc(i int, p []byte, alt bool) (int, Msg) {
